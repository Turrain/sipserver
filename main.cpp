...
private:
 ...
 std::string virtualAssistantInstruction = R"(Роль: Виртуальный оператор первой линии поддержки клиентов Береке банка
...
int main() {
     ...
     try {
        ...
        // Input user message and get chat completion
        std::string user_message = virtualAssistantInstruction;
        auto response = groq.getChatCompletion(user_message);
