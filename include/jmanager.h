...
class Logger
{
  public:
    ...
    void log(::LogLevel level, const std::string &message) {
        ...
        oss << "[" << timestamp << "] [" << coloredLevelStr << "] " << coloredMessage << "\n";
        std::string logMessage = oss.str();

        if (consoleLogging) { std::cout << logMessage; }
  ...
  private:
    ...
    std::mutex    mtx;
    Logger() : logLevel(::LogLevel::DEBUG), consoleLogging(true), fileLogging(false) {}
    ...
    std::string getColoredLevel(::LogLevel level, const std::string &levelStr) {
        ...
        if (it != LogLevelColors.end()) { return it->second + levelStr + ResetColor; }
        return levelStr;
    }

    std::string getColoredMessage(const std::string &message, ::LogLevel level) {
        auto it = LogLevelColors.find(level);
        if (it != LogLevelColors.end()) { return it->second + message + ResetColor; }
        return message;
    }
    ...
    std::string formatString(const char *format, va_list args) {
        ...
        if (length < 0) { return "Formatting error"; }
...
class jVAD
{
  ...
  public:
    ...
    void setSilenceCallback(SilenceCallback callback) { onSilence = std::move(callback); }

    void setVoiceFrameCallback(VoiceFrameCallback callback) { onVoiceFrame = std::move(callback); }
  ...
  private:
    ...
    void processVAD(const MediaFrame &frame, bool is_voiced) {
        if (!triggered)
        {
            ...
            if (num_voiced > VAD_RATIO * vadRingBuffer.size())
            {
                ...
                for (const auto &[f, s] : vadRingBuffer) { processVoicedFrame(f); }
                vadRingBuffer.clear();
            }
        } else
        {
            ...
            {
                if (onVoiceSegment && !voiceBuffer.empty()) { onVoiceSegment(voiceBuffer); }
                triggered = false;
                processSilence();
                voiceBuffer.clear();
                vadRingBuffer.clear();
    ...
    void processVoicedFrame(const MediaFrame &frame) {
        if (voiceBuffer.size() < MAX_BUFFER_SIZE) { voiceBuffer.push_back(frame); }
        if (onVoiceFrame) { onVoiceFrame(frame); }
    }
...
class jMediaPort : public AudioMediaPort
{
  public:
    ...
    void onFrameRequested(MediaFrame &frame) override { frame.type = PJMEDIA_FRAME_TYPE_AUDIO; }
...
class jAccount : public Account
{
  public:
    ...
    void registerRegStateCallback(onRegStateCallback cb) { regStateCallback = std::move(cb); }
...
class Manager
{
  ...
  public:
    ...
    void shutdown() {
        ...
        if (m_workerThread && m_workerThread->joinable()) { m_workerThread->join(); }
    }
    ...
    class TaskQueue
    {
      public:
        ...
        std::function<void()> dequeue() {
            ...
            if (stopped && tasks.empty()) { return nullptr; }
