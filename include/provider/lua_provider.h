...
public:
    ...
    ProviderResponse call_provider(const std::string& name,
        ...
        return it != providers_.end() ?
            it->second->send_request(input, options) :
            ProviderResponse{"Provider not found", {}};
    }
    ...
    void initialize_lua_bindings() {
        ...
        lua_["http_post"] = [](const std::string& url,
            ...
            if (res) {
                ...
                std::cerr << "HTTP Request Failed: " << res.error() << "\n";
    ...
    void load_providers() {
        for (const auto& [name, pc] : config_.providers()) {
            ...
            try {
                ...
                std::cout << "Loaded provider: " << name << "\n";
            }
            catch (const sol::error& e) {
                std::cerr << "Error loading provider " << name << ": " << e.what() << "\n";
            }
        }
    }
